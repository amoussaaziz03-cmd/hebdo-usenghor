...
//  Necessite la variable d'environnement GEMINI_API_KEY (gratuite sur aistudio.google.com)
// ══════════════════════════════════════════════════════
app.post('/scan-repas', async (req, res) => {
  try {
        ...
        body: JSON.stringify({
          contents: [{
            parts: [
              ...
              { text: 'Analyse cette photo de repas. Reponds en JSON avec ce format exact : {"aliments":["nom1","nom2"],"kcal":nombre,"proteines":nombre,"glucides":nombre,"lipides":nombre,"confiance":"haute|moyenne|basse"}. Les valeurs nutritionnelles sont des estimations pour la portion visible entiere.' }
            ]
          }],
    ...
    if (data.error) return res.status(502).json({ error: data.error.message || 'Erreur API Gemini' });
